...
append_table30v2_card_defaults \
  "run1-put-books-back-table30v2-50000step-1card-bs8" \
  "run1-put-books-back-table30v2-50000step-8card-bs256" \
  "$@"

cd "$PROJECT_ROOT"
exec "$PYTHON_BIN" -m robo_train.cli.train --framework kai0 pi05_put_the_books_back_table30v2 "${TABLE30V2_DEFAULT_ARGS[@]}" "$@"
